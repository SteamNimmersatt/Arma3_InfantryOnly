...
if (!isNil "INFONLY_enable" && {!INFONLY_enable}) exitWith {
	[INFONLY_LOGLEVEL_DEBUG, "Mod is disabled. Skipping vehicle spawn handling."] call infonly_main_fnc_log;
};

// Validate vehicle
if (isNull _vehicle) exitWith {
	[INFONLY_LOGLEVEL_DEBUG, "Null vehicle passed to spawn handler. Skipping."] call infonly_main_fnc_log;
};

// Skip infantry units (we only want to disable weapons on actual vehicles)
if (_vehicle isKindOf "Man") exitWith {
	[INFONLY_LOGLEVEL_DEBUG, "Infantry unit passed to spawn handler. Skipping."] call infonly_main_fnc_log;
};
...
if (!isNil {_vehicle getVariable "INFONLY_weaponsDisabled"}) exitWith {
	[INFONLY_LOGLEVEL_DEBUG, format ["Vehicle %1 has already been processed. Skipping.", typeOf _vehicle]] call infonly_main_fnc_log;
};

// Check if vehicle type is allowed
if ([_vehicle] call infonly_main_fnc_isVehicleTypeAllowed) exitWith {
	[INFONLY_LOGLEVEL_DEBUG, format ["Vehicle of type '%1' is allowed to keep ammunition. Skipping vehicle spawn handling.", typeOf _vehicle]] call infonly_main_fnc_log;
...
if (!local _vehicle) exitWith {
	[INFONLY_LOGLEVEL_DEBUG, format ["Vehicle %1 is not local. Skipping.", typeOf _vehicle]] call infonly_main_fnc_log;
};

[INFONLY_LOGLEVEL_DEBUG, format ["Disabling weapons on newly spawned vehicle: %1", typeOf _vehicle]] call infonly_main_fnc_log;
